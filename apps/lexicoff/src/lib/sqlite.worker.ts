...
async function search(lang: string, query: string, phoneticQuery: string): Promise<SearchResult[]> {
	...
	// True when the query contains no Greek characters — user is searching by English gloss,
	// not by Greek phonetic spelling. Used to demote phonetic coincidences below gloss matches.
	const isLatinQuery = !/\p{Script=Greek}/u.test(trimmed);
				...
				for (const [id, word, pos, freq] of phoneticRows) {
					...
					if (!seen.has(key)) {
						seen.set(key, {
							...
							pos: pos as string,
							matched: word as string,
							// For Latin queries (e.g. "angry"), phonetic coincidences with Greek words
					// should rank below genuine English gloss matches, not above them.
	...
	if (glossEntries.length > 0) {
		...
		try {
			...
			// best-effort — sort will fall back to freq
		}
	}
	...
	const top = [...byWord.values()]
		.sort(
			(a, b) =>
				a.quality - b.quality ||
				(a.quality === 3 ? (preGlossMatchIdx.get(a.id) ?? 9999) - (preGlossMatchIdx.get(b.id) ?? 9999) : 0) ||

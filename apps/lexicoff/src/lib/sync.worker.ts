	...
	async function doSync() {
		...
		try {
			...
			if (!local || local.hash !== remote.dataHash) {
				...
				await db.transaction('rw', [db.entries, db.metadata], async () => {
					await db.entries.where({ lang: lang }).delete();
					for (let i = 0; i < totalChunks; i++) {
						await db.entries.bulkPut(records.slice(i * CHUNK_SIZE, (i + 1) * CHUNK_SIZE));
						self.postMessage({
							type: 'PROGRESS',
							lang,
							phase: 'installing',
				...
				console.log(`Inserted ${records.length} ${lang} entries. sync finished`);
			} else {

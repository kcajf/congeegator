...
<div class="picker-container" use:preventPickerSelection use:clickOutside={() => (isOpen = false)}>
	<button
		class="trigger"
			...
			{#each Object.values(manifest.languages).toSorted( (a, b) => a.name.localeCompare( b.name, undefined, { sensitivity: 'base' } ) ) as lang (lang.code)}
				<button
					...
					onclick={(event) =>
						select(
							lang.code,
							event.detail === 0 || (event instanceof PointerEvent && event.pointerType === 'mouse')
						)}
				>
					{lang.name}
				</button>
